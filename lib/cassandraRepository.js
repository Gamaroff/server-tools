...
module.exports = function (name, schema) {
	...
	var pks = _.filter(sortedSchema, function (item) {
		return item.primary;
	});
	var primaryKeys = new Array(pks.length);
	...
	for (var i = 0; i < sortedSchema.length; i++) {
		...
		if (sortedSchema[i].primary >= 0) {
			primaryKeys.splice(sortedSchema[i].primary, 1, sortedSchema[i]);
		}

		if (sortedSchema[i].index && !sortedSchema[i].primary) {
			_indexes.push(sortedSchema[i].id);
		}
	}
	...
	if (primaryKeys.length > 0) {
		...
		for (var i = 0; i < primaryKeys.length; i++) {
			...
			if (key.partitioning && !isComposite) {
				isComposite = true;
				primaryKey += '(';
			}

			if (!key.partitioning && isComposite) {
				primaryKey = primaryKey.substr(0, primaryKey.length - 2); // remove last comma and space
				primaryKey += ')';
			}

			if(primaryKey.substr(primaryKey.length - 1, 1) == ')'){
				primaryKey += ', ';
			}
	...
	self.createTable = function (client, callback) {
		...
		_client.execute(_createStatement, function (err, result) {
			if (err) {
				...
				// create indexes
				_.each(_indexes, function (index) {
					_client.execute('CREATE INDEX  ON ' + name + ' (' + index + ');', function (err, result) {
						if (err) {
							console.log(err);
						}
					});
