...
function BlobStorage() {
    ...
    var blobService = azure.createBlobService(azureConfig.storageAccountOrConnectionString, azureConfig.storageAccessKey, azureConfig.host);
                ...
                var uploadFile = function (dataLength) {
                    ...
                    blobService.createBlockBlobFromStream(container, blockBlobName, stream, dataLength, options, function (err, result) {
                        ...
                        if (err) {
                            ...
                            var dto = {
                                file: 'http://' + azureConfig.storageAccountOrConnectionString + '.blob.core.windows.net/' + container + '/' + blockBlobName
                            };
            ...
            if (!error) {
                ...
                blobService.createBlockBlobFromFile(container, file, path + file, options, function (err, result) {
                    if (err) {
                        ...
                        var dto = {
                            file: 'http://' + azureConfig.storageAccountOrConnectionString + '.blob.core.windows.net/' + container + '/' + file
                        };
